...
Retrieval Augmented Generation (RAG) as its name suggest is a technique used to augment/improve the output of the LLMs models to users questions, its used in several tasks but the main one is Question-Answering (QA). 

### **Problematic**:

* With stadealone LLM model, when a user ask a question, the model we'll try to answer using his prebious knowldge from the data he was trained on but in most cases this data is not sufficient or out-dated (we all know for example that gpt-3.5-turbo is only trained up to 2021) and so the returned answer may be wrong or inaccurate.
  
* A simple intuitive fix would be to put all necessary information directly into the prompt as context for the model before answering the question but this approach can't work because most LLMs have a limited context size and the more tokens are added to the prompt the more the model tends to forget some of the information, and we should not forget that such approach would be very expensive as most LLMs deployed charge according to input token count  

### **Solution**:

Its here where the RAG approach come into play:
...
* We store all the text chunks a Vector DB, this kind of databases allow us to retrieve elements based on a similarity score between them.
* Thus using RAG when a user ask a question, we first perform a similarity search on our vector DB and extract only the most relevant chunks of informations which are given as context to the LLM model which will need to formulate a correct answer based on them.

## Limitations of standard RAG

Although the RAG technique is very powerfull, it has some limitations:

* It's difficult to choose the correct chunk size when splitting the data into smaller parts, because if its short the model will not have enough context to anwser and in if its too long we'll endup in the same limitation faced by standealone LLM .i.e the model context size limit and cost.

* The same problem applies for the number of instances we must fetch from our vector store after the similarity search is performed, for example choosing only the first three instances might lead to incorrect answer as the correct context was in the fifth instance.

* Retrieved document could be irrelavent to the user question or the model could start halluctinating, in both cases the model will output a wrong answer.
...
* **Adaptive RAG** [paper](https://arxiv.org/abs/2403.14403): It consists in adding a router agent which is reponsible for routing the questions to different retrieval approaches, for example if the asked question is related to our local vector store then the local RAG we'll give the answer, if not then a web search agent we'll handle it.

* **Corrective RAG** [paper](https://arxiv.org/pdf/2401.15884): This one introduces a fallback mechanism to answer the question when the retrieved documents for the local vector store are irrelevant to the question asked.

* **Self-RAG** [paper](https://arxiv.org/abs/2310.11511): develops an answer and hallucination grader agents, which evaluate the output response and regenerate the answers when the agent hallucinate or doesn’t address the question correctly.
...
Let's explore how our local RAG systme will operate, after the user ask his question we'll get the following steps:
...
3. **Document Grading**: Grading of retrieved documents from local vector store as relevant or irrelevant is performed.
...
5. **Response Generation**:
   ...
   - In case of irrelevant context, a web search is performed to gather suitable content.
...
* To construct the local data store for the research assistant you must first download research papers. Don't worry, I already setup the `scripts/downloader.py` script for that you just need to add your research topic and the number of papers to download, then run:

```bash
python scripts/downloader.py
```

* You got you research data, now you must convert it into vectors and store them into the Pinecone vector DB, you just have to run:
